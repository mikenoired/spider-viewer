...
export function GroupProgressSheet({
	...
	useEffect(() => {
		if (!open) {
			...
			setEffectiveDate(group.primaryRooms[0]?.effectiveDate ?? getTodayInMoscow())
		}
	}, [group, open])
	...
	const changedRooms = useMemo(
		() =>
			draftRooms.flatMap(room => {
				const source = group.primaryRooms.find(candidate => candidate.id === room.id)
								...
								<Popover>
									...
									<PopoverContent align="start" className="w-[min(22rem,calc(100vw-2rem))]">
										...
										<Calendar
											...
											onSelect={value =>
												setEffectiveDate(value ? format(value, "yyyy-MM-dd") : getTodayInMoscow())
											}
											className="w-full"
										/>
									</PopoverContent>
									...
									<TableRow
										...
										<TableCell
											...
											<div className="grid min-w-0 gap-3 sm:min-w-64 sm:grid-cols-[1fr_84px] sm:items-center">
												<Slider
													value={[room.progress]}
													onValueChange={value => updateRoomProgress(room.id, value[0] ?? 0)}
												...
												<Input
													...
													className="h-10 sm:h-8"
													onChange={event =>
														updateRoomProgress(room.id, Number(event.target.value))
													}
													disabled={!canEdit || pending}
												/>
											</div>
