...
Перед запуском контейнера Railway выполняет `node ./scripts/predeploy.mjs`: схема синхронизируется через Drizzle, затем создаются или обновляются 3 суперпользователя из `AUTH_SUPERUSERS_JSON`. Healthcheck смотрит на `/healthz`, чтобы деплой не зависел от прогрева базы или Redis.
