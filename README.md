...
## Что есть в проекте
- Авторизация с ролями `user`, `admin`, `super-admin`
- Импорт workbook (`.ods`, `.xlsx`, `.xls`) с обработкой листа `Общ`
- Интерактивная карта демонтажа с прогрессом по помещениям
- Аудит изменений в `Postgres + Redis`
- Отдельный журнал изменений задним числом и экспорт `.docx`
